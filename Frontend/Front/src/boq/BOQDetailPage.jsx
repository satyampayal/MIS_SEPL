...
export default function BOQDetailPage() {
  ...
  const fetchBOQDetails = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/boq/${boqId}`);
  ...
  const handleSaveItem = async () => {
    try {
      ...
      const url =
        mode === "add"
          ? `http://localhost:5000/boq/${boqId}/item`
          : `http://localhost:5000/boq/item/${selectedItem._id}`;
  ...
  const handleDeleteItem = async (itemId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/boq/item/${itemId}`, {
        method: "DELETE",
      });
  ...
  const handleUploadExcel = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/boq/${boqId}/upload-excel`,
        {
          method: "POST",
          body: form,
        }
